...
struct Vector2
{
	...
	//the operator overloads reutrn a new vector
	//not sure if this is the best way for this
	Vector2 operator + (Vector2 v2)
	{
		Vector2 ret(this->x_pos + v2.x_pos, this->y_pos + v2.y_pos);
		return ret;
	}
	Vector2 operator - (Vector2 v2)
	{
	...
	{
		Vector2 ret(this->x_pos * scale, this->y_pos * scale);
		return ret;
	}
	...
	float cross_mult(Vector2 v2) // returns cross product
	{
		return (this->x_pos * v2.y_pos) - (v2.x_pos * this->y_pos);
	}
	float dot_mult(Vector2 v2) // returns dot product
	{
		return (this->x_pos * v2.x_pos) + (v2.y_pos * this->y_pos);
	}
	void zero() //sets components to zero
	{
		x_pos = 0;
		y_pos = 0;
	}
	void one() //sets components to one
	{
		x_pos = 1;
		y_pos = 1;
	}
